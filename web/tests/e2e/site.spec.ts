  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /jest/api/. It is produced by the Go doc generator at deploy time,
        ...
        expect(resolved.pathname, `docs link should resolve under /jest/`).toBe('/jest/api/');
      } else {
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // jest is a single-package module (package jest), so at least one package link is expected.
